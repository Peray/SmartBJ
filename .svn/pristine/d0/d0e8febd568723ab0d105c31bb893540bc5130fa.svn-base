	...
	e.extend(e.fx.step, {
		backgroundPosition: function(b) {
			...
			if (0 === b.state && "string" == typeof b.end) {
				var d = e.curCSS(b.elem, "backgroundPosition"),
				d = f(d);
				b.start = [d[0], d[2]];
				d = f(b.end);
				b.end = [d[0], d[2]];
...
$(function() {
	function e() {
		...
		var a = c.split(""),
		b = ["numf", "nums", "numt", "numfo"];
		for (i = 0; 4 > i; i++)
			$("." + b[i] + "").stop(!0, !1).animate({ "background-position-x": "0px", "background-position-y": -24 * a[i] }, 200)
	}
	...
	var h = f - row;
    // $(".list").height(220*row);
    // $("#content").height(220*row+25);
    $.fn.liOut = function() {
    	$(this).find(".lileft").animate({ left: "-80px" }, 1000, "easeOutQuart");
    	$(this).find(".liright").animate({ left: "500px" }, 1000, "easeOutQuart")
    };
    $.fn.liIn = function() {
    	$(this).find(".lileft").animate({ left: "0px" }, 1000, "easeOutQuart");
    	$(this).find(".liright").animate({ left: "130px" }, 1000, "easeOutQuart")
    };
    $(".arrowdown").click(function() {
    	a && parseInt($(".list li:first").css("marginTop")) > -220 * h && (d++, b--, a = !1, $(".list li").eq(d - 1).liOut(), $(".list li").eq(d + row - 1).liIn(), $(".list li:first").animate({ marginTop: 220 * b }, 600, "easeInOutQuad", function() {
    		a = !0;
    ...
    $(".arrowup").click(function() {
    	a && 0 != parseInt($(".list li:first").css("marginTop")) && (b++, d--, a = !1, $(".list li").eq(d).liIn(), $(".list li").eq(d + row).liOut(), $(".list li:first").animate({ marginTop: 220 * b }, 600, "easeInOutQuad", function() {
    		a = !0;
    ...
    $(".list").mousewheel(function(b, c) {
    	...
    	if (a) {
    		...
    		0 < c ? 0 != d && ($(".arrowup").addClass("arrow_active"), $(".arrowup").trigger("click")) : d > -220 * h && ($(".arrowdown").addClass("arrow_active"), $(".arrowdown").trigger("click"))
    	}
    });
    $(".timeblock").attr("thisyear", c);
    $(".list li:gt(" + (row - 1) + ")").find(".lileft").css({ left: "-80px" });
    $(".list li:gt(" + (row - 1) + ")").find(".liright").css({ left: "500px" });
